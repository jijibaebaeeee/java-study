...
//			System.out.println("null값은 입력 불가능합니다.");
			//예외정보를 얻는 방법 3가지
			//1. e.toString(), e => 예외 종류(클래스) + 발생한 이유 반환0
//			System.out.println(e);
//			System.out.println(e.toString());
			//2. e.getMessage() => 예외 발생한 이유만 반환
//			System.out.println(e.getMessage());
			//3. e.printStackTrace() -> 예외가 어디서 발생했는지 추적한 내용까지 출력
			e.printStackTrace(); // 평소에 출력되는 내용이 얘 때문임
			
		}catch(Exception e) {
			// 우리가 try에서 어떤 오류가 발생할 지 전부 예상하기 힘들다
			// 예상치 못한 오류를 잡기 위해 Exception타입의 매개변수를 사용한다
			// 모든 예외클래스의 조상은 Exception 크래스이다.
			// 즉, 어떤 예외가 발생하더라고 Exception타입의 참조변수에 저장가능하다(업캐스팅)
