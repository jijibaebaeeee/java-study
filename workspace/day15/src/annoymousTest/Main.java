...
//		InterA ia = new ClassA(); //이전까지 알던 형태

		InterA ia = new InterA(){ // 익명클래스 -> 로컬내부클래스
		...
		// 인터페이스는 객체로 생성할 수 없어서 익명클래스를 통해 인터페이스의
		// 메소드를 구현하고 객체로 생성할 수 있다
		
		// 익명클래스는 : 자식(하위타입)
		// 익명클래스는 InterA 인터페이스를 자동으로 implements한다
		// 익명클래스는 작성된 시점에 하나의 구체적인 클래스로 동작한다.
		...
		// 부모타입으로 참조하고 있지만 실제 메소드 호출시에는 오버라이딩 된 자식클래스의 메소드가 실행된다(다형성)
